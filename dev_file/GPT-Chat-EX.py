...
async def chat_with_gpt(request: ChatRequest):
    try:
        ...
        key_source = "외부 API 키 사용" if request.api_key else "환경 변수 API 키 사용"
        ...
        if request.is_continuous:
            ...
            # 히스토리가 너무 길어지는 것을 방지
            if len(conversations[request.conversation_id]) > 20:  # 최대 10턴으로 제한
                conversations[request.conversation_id] = conversations[request.conversation_id][-20:]
        ...
        if "invalid_api_key" in error_str:
            ...
            return {"error": "서버에서 오류가 발생했습니다. 잠시 후 다시 시도해주세요."}
